...
class MacenkoStainNormalizer(StainNormalizer):
    ...
    def __init__(
        ...
        # ---- validate beta ----
        if not (0 < beta < 3):
            raise ValueError(f"beta must be between 0 and 3. Got {beta}.")
        # ---- validate percentile ----
        ...
        self.cos_similarity_max = float(
            cos_similarity_max
        )  # if cos(stain1, stain2) > this, widen alpha
        self.scale_clip = (
            float(scale_clip[0]),
            float(scale_clip[1]),
        )  # clamp per-stain scale factors
    ...
    def _sanitize_basis(self, W: np.ndarray) -> np.ndarray:
        ...
        if cos_sim > self.cos_similarity_max:
            if self.verbose:
                print(
                    f"[Macenko] Stains too similar (cos={cos_sim:.3f}), enforcing PCA-orthogonal fallback."
                )
    ...
    def fit(
        self,
        target_tile: np.ndarray | list[np.ndarray],
    ...
    def fit_source(
        self, source: Any, thumb_size: int = 1024
    ) -> "MacenkoStainNormalizer":
        """
        ...
        if mask.mean() < 0.05:  # less than 5% tissue pixels
            raise RuntimeError(
                "Source slide has too little tissue for reliable fitting."
            )
    ...
    def normalize(self, source_tile: np.ndarray) -> np.ndarray:
        ...
        if self.W_src is not None:
            ...
            if not self.allow_fallback:
                raise RuntimeError(
                    "W_src not fitted at slide-level. "
                    "Call fit_source(slide) before normalize()."
                )

            if self.verbose:
                print(
                    "[Macenko] Warning: W_src not fitted at slide-level. "
        ...
        if self.verbose:
            for j, (src, tgt, s, sc) in enumerate(
                zip(H_src_pct, self.H_target_pct, scale, scale_clipped)
            ):
                print(
                    f"[Macenko/normalize] Channel {j}: H_src_pct={src:.4f}, "
    ...
    def get_profile(self) -> Dict[str, Any]:
        return {
            ...
            "H_target_pct": None
            if self.H_target_pct is None
            else self.H_target_pct.tolist(),
        }
    ...
    def _estimate_stain_matrix(self, I: np.ndarray) -> np.ndarray:
        ...
        if cos_sim(W[:, 0], W[:, 1]) > self.cos_similarity_max:
            if self.verbose:
                print(
                    f"[Macenko] Stains too similar with alpha={self.alpha}, widening..."
                )
