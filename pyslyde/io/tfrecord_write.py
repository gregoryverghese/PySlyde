...
def getShardNumber(images, masks, shardSize=0.01, unit=10**9):
    ...
    print(
        "Image memory: {}, Mask memory: {}, Total memory: {}".format(
            imageMem, maskMem, totalMem
        )
    )
...
def doConversion(imgs, masks, shardNum, num, outPath, outDir):
    ...
    for i in range(0, shardNum):
        ...
        convert(
            shardImgs,
            shardMasks,
            os.path.join(outPath, outDir, str(i) + ".tfrecords"),
            dim=None,
        )
...
def getFiles(imagePath, maskPath, outPath, config, shardSize=0.1):
    ...
    trainImgs = [
        ...
        if not any([v for v in validFiles + testFiles if v in img])
    ]
    trainMasks = [
        m for m in maskPaths if not any([v for v in validFiles + testFiles if v in m])
    ]
    ...
    print(
        "train:{}, valid: {}, test: {}".format(
            len(trainImgs), len(validImgs), len(testImgs)
        )
    )
    print(
        "train:{}, valid: {}, test: {}".format(
            len(trainMasks), len(validMasks), len(testMasks)
        )
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-op", "--outpath", required=True, help="path for tfRecords to be wrriten to"
    )
