...
def test_gated_hf_model_does_not_login_if_cached(monkeypatch):
    ...
    - Model construction proceeds without requiring HUGGINGFACE_TOKEN

    The actual model loader is patched with a minimal (trivial) TorchWrapper
    stand-in to avoid heavyweight downloads and isolate the gating behavior
    under test.
    """
    gated = next(iter(GATED_HF_MODELS))
    ...
    fg = FeatureGenerator(model_name=gated)
